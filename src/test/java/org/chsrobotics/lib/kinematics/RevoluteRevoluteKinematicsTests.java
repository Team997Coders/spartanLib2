...
public class RevoluteRevoluteKinematicsTests {
    @Test
    public void RevoluteRevoluteInverseKinematicsRejectsImpossible() {
        RevoluteRevoluteKinematics kinematics = new RevoluteRevoluteKinematics(1, 1);
        ...
        kinematics = new RevoluteRevoluteKinematics(2, 1);
    ...
    @Test
    public void RevoluteRevoluteInverseKinematicsWorks() {
        RevoluteRevoluteKinematics kinematics = new RevoluteRevoluteKinematics(2, 1);

        assertEquals(
                -Math.acos(0.875),
                kinematics.inverseKinematics(2, 0).firstValue().jointAAngle,
                UtilityMath.defaultAbsoluteEpsilon);
        assertEquals(
                Math.acos(0.875),
                kinematics.inverseKinematics(2, 0).secondValue().jointAAngle,
                UtilityMath.defaultAbsoluteEpsilon);

        assertEquals(
                Math.PI - Math.acos(0.25),
                kinematics.inverseKinematics(2, 0).firstValue().jointBAngle,
                UtilityMath.defaultAbsoluteEpsilon);

        assertEquals(
                Math.acos(0.25) - Math.PI,
                kinematics.inverseKinematics(2, 0).secondValue().jointBAngle,
                UtilityMath.defaultAbsoluteEpsilon);
    }

    @Test
    public void RevoluteRevoluteForwardKinematicsWorks() {
        RevoluteRevoluteKinematics kinematics = new RevoluteRevoluteKinematics(1, 1);
